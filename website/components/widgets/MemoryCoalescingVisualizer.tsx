...
export function MemoryCoalescingVisualizer() {
  ...
  return (
    <div className="my-6 rounded-lg border border-surface-border bg-surface-raised/40 p-5">
      <p className="mb-4 text-base font-semibold text-white">Memory Coalescing Visualizer</p>

      <div className="grid gap-4 sm:grid-cols-2">
        <label className="text-sm text-slate-300">
          Stride:<span className="ml-2 font-mono text-brand">{stride}</span>
          <input
            type="range"
            min={1}
            max={8}
        ...
        <label className="text-sm text-slate-300">
          Offset:<span className="ml-2 font-mono text-brand">{offset}</span>
          <input
            type="range"
            min={0}
            max={31}
      ...
      <div className="mt-5">
        <p className="mb-2 text-xs uppercase tracking-wide text-slate-400">32 threads → memory transactions</p>
      ...
      <dl className="mt-5 grid grid-cols-2 gap-3 text-sm sm:grid-cols-3">
        <div className="rounded-md bg-surface p-3">
          <dt className="text-xs text-slate-400">Transactions</dt>
          <dd className="font-mono text-lg text-white">{segments.length}</dd>
        </div>
        <div className="rounded-md bg-surface p-3">
          <dt className="text-xs text-slate-400">最佳情況</dt>
          <dd className="font-mono text-lg text-white">{Math.ceil((kWarpSize * stride) / kElementsPerSegment) === 1 ? 1 : Math.ceil(kWarpSize / kElementsPerSegment)}</dd>
        </div>
        <div className="rounded-md bg-surface p-3">
          <dt className="text-xs text-slate-400">Bandwidth efficiency</dt>
          <dd className={`font-mono text-lg ${efficiency > 0.9 ? 'text-brand' : efficiency > 0.4 ? 'text-[#ffa657]' : 'text-[#ff7b72]'}`}>
      ...
      <p className="mt-3 text-xs text-slate-400">
        當 <code className="text-brand">stride = 1</code> 且 offset 對齊時,32 個 threads 落在同一個 128-byte segment,只需 1 次 transaction,efficiency 達 100%。stride 越大,touched segments 越多,搬進來卻用不到的 bytes 就是浪費的 bandwidth。
      </p>
    </div>
  );
}
