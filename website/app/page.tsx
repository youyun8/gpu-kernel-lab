    ...
    <AppWidthContainer as="main" className="px-4">
      <section className="relative overflow-hidden rounded-2xl border border-border bg-gradient-to-br from-card via-accent to-card px-6 py-16 sm:px-12">
        ...
        <div className="mt-8 flex flex-wrap gap-3">
          ...
          <Link href="/roadmap" className="rounded-lg border border-border px-5 py-2.5 font-medium text-foreground transition hover:border-primary">
            查看學習路線圖
          </Link>
          <Link href="/exercises" className="rounded-lg border border-border px-5 py-2.5 font-medium text-foreground transition hover:border-primary">
            練習與解答
          </Link>
        </div>
      </section>
      ...
      <section className="mt-12">
        <h2 className="mb-2 text-2xl font-semibold text-foreground">10 分鐘快速開始</h2>
...
cd .. && python scripts/bench_all.py --out website/content/data/benchmarks.json`}
      ...
      <section className="mt-12 mb-16">
        ...
        <p className="mb-6 text-sm text-muted-foreground">Track 1 → 10。 前段 (平行化、memory hierarchy、入門到專家與 GEMM 深入) 是能力階梯, 後段把技巧延伸到 PyTorch、library 與 production kernel 戰場。</p>
        <LearningPathMap />
      </section>
    </AppWidthContainer>
  );
