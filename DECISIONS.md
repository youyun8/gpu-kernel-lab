...
## 2026-07-03 Track restructuring: Parallelization + Memory foundations

### 平行化基礎改為獨立 Track 0

使用者澄清需求後, 先前「把平行化與 data race 作為 Track A 第 0 章」的決策被 supersede。 新結構改為獨立 `Track 0 — 平行化基礎`, 章節依學習順序拆成: parallelization design、CPU threading primitives、data races/locks/atomics/reduction、GPU synchronization/debugging。 理由: 平行化基礎橫跨 CPU 與 GPU, 不應被包在 GPU 入門 track 裡; 拆成多章也能容納 mutex、semaphore、barrier、condition variable、memory order、fence 等必要概念。

### 新增 Track M: Memory Hierarchy

新增 `Track M — Memory Hierarchy`, 專門處理 DRAM/SRAM/cache/register 的硬體模型、cycle/bandwidth/roofline 紙上計算、locality/tiling/coalescing/bank conflict, 以及 memory optimization workflow。 理由: 既有 Track A/B 有 GPU memory 章節, 但使用者要求一條從基本硬體層級到計算與優化的完整 track; 獨立 track 可讓讀者先建立可手算的 performance model, 再進入 GPU-specific memory coalescing 與 shared memory 深水區。

### Exercise sets 跟著 foundations track 獨立

新增 `track-p` 與 `track-m` exercise sets, 都包含 paper-and-pencil 與 programming 題。 Paper 題覆蓋 interleaving、primitive 選型、latency/bandwidth/roofline/transaction/tile capacity 等可手算能力; programming 題連到 `kernels/00-parallel-foundations` 與 `kernels/02-memory` 的可執行 examples。 這比把 foundations 題混進 Track A/B 更清楚, 也避免 GPU 入門練習過度膨脹。
